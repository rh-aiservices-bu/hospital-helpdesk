...
description: "Install and configure RHOAI 3.5 on OpenShift so a regular user can complete a GenAI tutorial without admin blockers — channel pins, DSC component selection (aipipelines, mlflow), genAiStudio + promptManagement flags, model registry, S3 storage, workbench pip fix, MCP/OGX registration, minimal RBAC, verified gotchas"
---
...
5. Run the tutorial's workbench and notebooks — including the runtime `pip install llama-stack-client` in `2_rag.ipynb` / `rag_pipeline.py` (modules 3.4/5.2).
6. Save/load prompts (module 2.3) and see traces under *Experiments (MLflow)* (modules 4.2, 5.3, 6.2).
7. Run the MCP-ticketing flow: deploy the server AND get it registered in OGX so the Playground lists it (module 6).

## 0. Pre-install prerequisites (docs: Installing and uninstalling OAI, §3.1/§3.3)
Confirm BEFORE subscribing — several are only fixable pre-install:
- OpenShift 4.19–4.20 (4.20+ required for llm-d deployments); cluster-admin access (`kubeadmin` is NOT allowed); default storage class with dynamic provisioning (`oc get storageclass`); an IdP configured; Open Data Hub NOT installed.
- **OGX/RAG workloads: install the Red Hat OpenShift Service Mesh Operator 3.x AND the cert-manager Operator BEFORE the RHOAI operator** (docs explicit for Llama Stack/RAG; adding them later means tearing down).
- GPU path additionally: GPU-enabled nodes + Node Feature Discovery + NVIDIA GPU Operator.
- Cluster floor: 2 worker nodes ≥8 CPU / 32 GiB RAM each (single-node: ≥32 CPU / 128 GiB).
...
## 2. DSC — component selection (the tutorial breaks without aipipelines + mlflow)
```bash
...
spec:
  components:
    ...
    aipipelines:             # 3.x key — 2.x called it datasciencepipelines; modules 1.3/5.2 need it
      managementState: Managed
    kserve:
      managementState: Managed
    workbenches:
    ...
    trustyai:
      managementState: Managed
    modelregistry:           # add registriesNamespace: rhoai-model-registries if you need the custom ns
      managementState: Managed
    ogx:                     # ships gen-ai-ui + llama-stack operator (tutorial playground)
      managementState: Managed
    mlflowoperator:          # REQUIRED — prompts (2.3) + Experiments (MLflow) traces (4.2/5.3/6.2) live in MLflow
      managementState: Managed
    llamastackoperator:      # DEPRECATED in 3.5 — must be Removed before ogx (operator guardrail); absent from the 3.5 docs component list, keep Removed only if present on your cluster
      managementState: Removed
EOF
```
**Schema drift note:** the 3.5 docs' `DataScienceCluster` examples use `apiVersion .../v2` with `aipipelines` (and `mcplifecycleoperator`, `feastoperator`) — no `llamastackoperator`, no `datasciencepipelines`. The v1 YAML above still parses on 3.5.0, but expect the field names to move on upgrades.
**Guardrails (verified):**
...
- A single `DSCInitialization` named `default-dsci` is auto-created by the operator in 3.5 — do NOT create one; creating another is denied ("Only one instance allowed"). Verify instead:
```bash
...
## 4. Enable Gen AI Studio + prompt management (Technology Preview) in the dashboard
The model catalog UI (search bar + model tiles on AI hub → Models) is hidden by default, and prompt save/load (module 2.3) is gated by its own flag (`promptManagement`, default false). Patch the CR **and restart the dashboard** — the operator does not propagate these flags to the running backend without a restart (verified).
```bash
oc patch odhdashboardconfig odh-dashboard-config -n redhat-ods-applications \
  --type=merge -p '{"spec":{"dashboardConfig":{"genAiStudio":true,"genAiTracing":true,"promptManagement":true}}}'
...
## 7. Workbench pip index fix (llama-stack-client — verified gap)
The 3.5 CPU workbench pip index (packages.redhat.com public-rhai/rhoai/3.5/cpu-ubi9) does NOT carry `llama-stack-client`, even though the workbench image ships it. The tutorial installs it at runtime (`2_rag.ipynb`, `rag_pipeline.py` — `!pip install llama-stack-client`), so that cell FAILS on a fresh 3.5 workbench.
- Tell users to install with `--index-url https://pypi.org/simple/`, or
- Pre-inject for the whole project: create an `OdhPipelineRuntime`-style env or simply document the override in the handoff note.
No admin CR change needed — this is a doc/handoff fix, but without it modules 4/5 die mid-notebook.

## 8. User RBAC (so the user can use the Playground)
...
## 9. ConfigMap the Playground needs (verified 404 without it)
```bash
oc create -f - <<'EOF'
apiVersion: v1
kind: ConfigMap
...
| 4 | DSCI already exists | "Only one instance allowed" | operator auto-creates `default-dsci`; don't create another |
...
| 9 | Missing `gen-ai-aa-mcp-servers` ConfigMap | Playground hangs on Loading | create the ConfigMap (step 9) |
...
| 18 | RedHatAI HF source is public-non-gated only | tutorial's `*-quantized.w8a8` tiles may never appear in the catalog (gated upstream, 401) | add the RedHatAI source early in Settings → Model resources and operations → Model catalog settings; expect granite fallback tiles; don't chase the missing tiles |

## Uninstall procedure (for a clean re-install)
**Official 3.5 method (docs, CLI-recommended — the web console can leave components behind):**
```bash
...
# wait for redhat-ods-applications to disappear, then:
...
oc get dsci default-dsci -o jsonpath='{.metadata.finalizers}'
oc patch dsci default-dsci --type=merge -p '{"metadata":{"finalizers":null}}'
...
```
**`Auth` CR gotcha (docs §10.7):** reinstall fails with "unable to find DSCInitialization" if the `Auth` custom resource (`services.platform.opendatahub.io`) from the previous install wasn't deleted. Delete it before reinstalling (`oc delete auth default` or via API Explorer). Also purge `FeatureTracker` instances. Note: admin/user dashboard groups now live in that `Auth` resource — `spec.groupsConfig` in OdhDashboardConfig is no longer used.

## Handoff checklist (before telling the user to start)
- [ ] `oc get dsc` → Ready
- [ ] Dashboard URL loads; user can log in
- [ ] `genAiStudio` + `promptManagement` true in `oc get odhdashboardconfig odh-dashboard-config -n redhat-ods-applications -o jsonpath='{.spec.dashboardConfig}'`
